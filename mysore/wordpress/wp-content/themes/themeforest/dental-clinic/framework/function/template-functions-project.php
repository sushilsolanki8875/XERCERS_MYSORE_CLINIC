...
function cmsms_project_check_exc_cont() {
	$exc = cmsms_project_exc_cont(false);
		...
		if ($template_type == 'page') {
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_cat']) {
				$out .= '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Categories', 'dental-clinic') . ':' . '</div>' . 
...
function cmsms_project_like($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_like']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Like', 'dental-clinic') . ':' . '</div>' . 
	...
	if (comments_open()) {
		if ($template_type == 'page') {
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_comment']) {
				$out .= '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Comments', 'dental-clinic') . ':' . '</div>' . 
...
function cmsms_project_date($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_date']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Date', 'dental-clinic') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function cmsms_project_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsms_project_author">' . 
			...
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'dental-clinic') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($template_type == 'post') {
		$cmsms_option = cmsms_get_global_options();
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_author']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Author', 'dental-clinic') . ':' . '</div>' . 
				'<div class="project_details_item_desc vcard author">' . 
					'<span class="cmsms_project_author fn" title="' . esc_attr__('Projects by', 'dental-clinic') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span>' . 
				'</div>' . 
			'</div>';
		}
	}
	...
	if (get_the_terms($cmsms_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_tag']) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Tags', 'dental-clinic') . ':' . '</div>' . 
					'<div class="project_details_item_desc">' . 
...
function cmsms_project_link($link_text, $link_url, $link_target, $show = true) {
	...
	if ( 
		...
		$out = '<div class="project_details_item">' . 
			'<div class="project_details_item_title">' . esc_html__('Project Link', 'dental-clinic') . ':' . '</div>' . 
			'<div class="project_details_item_desc">' . 
				'<span class="cmsms_project_tags">' . 
