...
function cmsms_get_settings() {
	...
	case CMSMS_PAGE_BASENAME:
		...
		$cmsms_settings_page_title = esc_html__('Theme General Settings', 'dental-clinic');
	...
	case CMSMS_PAGE_BASENAME . '-font':
		...
		$cmsms_settings_page_title = esc_html__('Theme Fonts Settings', 'dental-clinic');
	...
	case CMSMS_PAGE_BASENAME . '-color':
		...
		$cmsms_settings_page_title = esc_html__('Theme Color Schemes', 'dental-clinic');
	...
	case CMSMS_PAGE_BASENAME . '-element':
		...
		$cmsms_settings_page_title = esc_html__('Theme Elements Settings', 'dental-clinic');	
		$cmsms_page_sections = cmsms_options_element_sections();
		$cmsms_page_fields = cmsms_options_element_fields();
		$cmsms_page_tabs = cmsms_options_element_tabs();
	...
	case CMSMS_PAGE_BASENAME . '-single':
		...
		$cmsms_settings_page_title = esc_html__('Theme Single Posts Settings', 'dental-clinic');
	...
	case CMSMS_PAGE_BASENAME . '-demo':
		...
		$cmsms_settings_page_title = esc_html__('Theme Settings Import/Export', 'dental-clinic');
...
function cmsms_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		__('Default Field', 'dental-clinic'), 
		'desc' => 		__('This is a default description.', 'dental-clinic'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function cmsms_settings_scripts() {
	...
	wp_localize_script('cmsms_theme_settings_js', 'cmsms_setting', array( 
		...
		'remove_sidebar' => 	esc_html__('Do you realy want to remove this sidebar?', 'dental-clinic'), 
		'find' => 				esc_attr__('Find icons', 'dental-clinic'), 
		'remove_icon' => 		esc_html__('Do you realy want to remove this social icon?', 'dental-clinic'), 
		'theme_uri' => 			get_template_directory_uri(), 
		'shortname' => 			CMSMS_SHORTNAME, 
		'done' => 				esc_html__('All theme settings were imported successfully.', 'dental-clinic'), 
		'fail' => 				esc_html__("Error on page!!!\nPlease reload page and try again.", 'dental-clinic') 
	));
...
function cmsms_add_menu() {
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'dental-clinic'), 
		esc_attr__('Theme Settings', 'dental-clinic'), 
		'manage_options', 
		CMSMS_PAGE_BASENAME, 
	...
	$cmsms_settings_general = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme General Settings', 'dental-clinic'), 
		esc_attr__('General', 'dental-clinic'), 
		'manage_options', 
		CMSMS_PAGE_BASENAME, 
		'cmsms_settings_page_fn' 
	);
	...
	$cmsms_settings_font = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Fonts Settings', 'dental-clinic'), 
		esc_attr__('Fonts', 'dental-clinic'), 
		'manage_options', 
	...
	$cmsms_settings_color = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Color Schemes', 'dental-clinic'), 
		esc_attr__('Colors', 'dental-clinic'), 
		'manage_options', 
	...
	$cmsms_settings_element = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Elements Settings', 'dental-clinic'), 
		esc_attr__('Elements', 'dental-clinic'), 
		'manage_options', 
		CMSMS_PAGE_BASENAME . '-element', 
		'cmsms_settings_page_fn' 
	);
	
	add_action('load-' . $cmsms_settings_element, 'cmsms_settings_scripts');
	
	
	$cmsms_settings_single = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Single Posts Settings', 'dental-clinic'), 
		esc_attr__('Single Posts', 'dental-clinic'), 
		'manage_options', 
	...
	$cmsms_settings_demo = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Settings Import/Export', 'dental-clinic'), 
		esc_attr__('Import/Export', 'dental-clinic'), 
		'manage_options', 
...
function cmsms_settings_page_fn() {
	...
	if ($current_tab != 'wpml') {
		...
		'<p class="submit-top">' . 
			(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'dental-clinic') . '" />' : '') . 
		'</p>';
		...
		echo '<p class="submit">' . 
				(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'dental-clinic') . '" />' : (($current_tab == 'import') ? '<input name="import" type="button" class="button button-primary button-large cmsms-demo-import" value="' . esc_attr__('Import Settings', 'dental-clinic') . '" />' : '')) . 
			'</p>' . 
		'</form>';
	} else {
		do_settings_sections(__FILE__);
...
function cmsms_form_field_fn($args = array()) {
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'dental-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($google_font) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'dental-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsms_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'dental-clinic') . ' <em>(' . esc_html__('in pixels', 'dental-clinic') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsms_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'dental-clinic') . ' <em>(' . esc_html__('in pixels', 'dental-clinic') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'dental-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'dental-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'dental-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'dental-clinic') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'dental-clinic') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'wpml':
		echo '<div class="wrap otgsi_downloads_form">' .
			'<span>' . __("This plugin makes it easy to build multilingual sites and run them. It's powerful enough for corporate sites, yet simple for blogs.", 'dental-clinic') . '</span>';
	
		// CONFIG
		$config['template'] = 'compact'; //required
		$config['product_name'] = 'WPML'; 
		$config['box_title'] = 'Multilingual Dental Clinic';
		$config['name'] = 'Dental Clinic'; //name of theme/plugin 
		$config['box_description'] = 'Dental Clinic theme is fully compatible with WPML - the WordPress Multilingual plugin. WPML lets you add languages to your existing sites and includes advanced translation management.'; 
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsms_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'dental-clinic') . '" />' . 
				'<a href="#" class="cmsms_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'dental-clinic') . '"></a>' . 
			'</p>' . 
			...
			'<span class="icon_upload_link" style="display:none;">' . 
				'<label for="new_icon_link">' . 
					...
					__('Icon Link', 'dental-clinic') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					__('Icon Title', 'dental-clinic') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					__('Open link in a new tab/window?', 'dental-clinic') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'dental-clinic') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'dental-clinic') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'dental-clinic') . '"></a> ' . 
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Expecting a Numeric value!', 'dental-clinic');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						CMSMS_SHORTNAME . '_txt_numeric_error',
						__('Expecting a Numeric value! Please fix.', 'dental-clinic'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : esc_html__('Expecting comma separated numeric values', 'dental-clinic');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						...
						__('Expecting comma separated numeric values! Please fix.', 'dental-clinic'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : esc_html__('Invalid email! Please re-enter!', 'dental-clinic');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = esc_html__('This setting field cannot be empty! Please enter a valid email address.', 'dental-clinic');
				}
				...
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						...
						__('Please enter a valid email address.', 'dental-clinic'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Number!', 'dental-clinic');
...
function theme_admin_bar_render() {
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Theme Settings', 'dental-clinic'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('General', 'dental-clinic'), 
		'href' => esc_url(admin_url('admin.php?page=cmsms-settings')) 
	));
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Fonts', 'dental-clinic'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Colors', 'dental-clinic'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Elements', 'dental-clinic'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Single', 'dental-clinic'), 
...
function cmsms_add_global_options() {
	$cmsms_option_names = array( 
		...
		array( 
			...
			cmsms_options_general_fields('bg') 
		), 
		array( 
			'cmsms_options_' . CMSMS_SHORTNAME . '_header', 
			cmsms_options_general_fields('header') 
		),
		array( 
			'cmsms_options_' . CMSMS_SHORTNAME . '_content', 
			cmsms_options_general_fields('content') 
		), 
		array( 
			'cmsms_options_' . CMSMS_SHORTNAME . '_footer', 
			cmsms_options_general_fields('footer') 
		), 
		array( 
		...
		array( 
			...
			cmsms_options_element_fields('sidebar') 
		), 
		array( 
			'cmsms_options_' . CMSMS_SHORTNAME . '_element_icon', 
			cmsms_options_element_fields('icon') 
		), 
		array( 
			'cmsms_options_' . CMSMS_SHORTNAME . '_element_lightbox', 
			cmsms_options_element_fields('lightbox') 
		), 
		array( 
			'cmsms_options_' . CMSMS_SHORTNAME . '_element_sitemap', 
			cmsms_options_element_fields('sitemap') 
		), 
		array( 
			'cmsms_options_' . CMSMS_SHORTNAME . '_element_error', 
			cmsms_options_element_fields('error') 
		), 
		array( 
			'cmsms_options_' . CMSMS_SHORTNAME . '_element_code', 
			cmsms_options_element_fields('code') 
		), 
		array( 
			'cmsms_options_' . CMSMS_SHORTNAME . '_element_wpml', 
