...
function cmsms_admin_enqueue_scripts($hook) {
	if ( 
		...
		wp_localize_script('cmsms_theme_options_js', 'cmsms_options', array( 
			'create_gallery' => 	esc_attr__('Create Gallery', 'dental-clinic'), 
			'select_format' => 		esc_attr__('Please select the format.', 'dental-clinic'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'dental-clinic'), 
			'want_remove' => 		esc_html__('Do you realy want to remove this item?', 'dental-clinic'), 
...
function show_cmsms_meta_box() {
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'dental-clinic') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'dental-clinic') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'dental-clinic') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'dental-clinic') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'dental-clinic') . '</option>';
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsms_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'dental-clinic') . '" />' . 
					'<a href="#" class="cmsms_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'dental-clinic') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'dental-clinic') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'dental-clinic') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . esc_attr__('Blank Field', 'dental-clinic') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'dental-clinic') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . esc_attr__('Posts', 'dental-clinic') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Pages', 'dental-clinic') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Projects', 'dental-clinic') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'dental-clinic') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'dental-clinic') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'dental-clinic') . ' &nbsp;</option>';
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsms_gallery_item">' . 
							...
							'<a href="#" class="cmsms_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'dental-clinic') . '"></a>' . 
						'</li>';
					}
				}
			}
...
function add_custom_cmsms_meta_box() {
    add_meta_box( 
		...
		__('Theme Options', 'dental-clinic'), 
		'show_cmsms_meta_box', 
		'', 
		'normal', 
		'high' 
