 ...
 * @version		1.0.0
 * 
 * Blog Page Puzzle Status Post Format Template
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsms_puzzle_type'); ?> data-category="<?php echo esc_attr($post_categs); ?>">
	...
	<?php 
		if (!post_password_required() && has_post_thumbnail()) {
			...
			cmsms_post_format_icon_placeholder(get_the_ID(), 'person');
		}
		...
		echo '<div class="puzzle_post_content_wrapper">' . 
			...
			if (!post_password_required()) {
				if ($cmsms_post_status_text != '') {
					...
					echo '<div class="cmsms_post_content">' . 
						'<div class="entry-title entry-content">' . theme_excerpt(20, false) . '</div>' . 
					'</div>';
				}
			} else {
