...
final class WP_Installer{
    ...
    public function menu_setup(){
        ...
        if(is_multisite() && !is_network_admin()){
			$add_options = 'add_options_';
			
			$add_options_p = $add_options . 'page';
			
            $this->menu_multisite_redirect();
            $add_options_p(__('Installer', 'installer'), __('Installer', 'installer'), 'manage_options', 'installer', array($this, 'show_products'))            ;
        }else{
    ...
    public function menu_url(){        
        if(is_multisite()){
            if(is_network_admin()){
                $url = esc_url(network_admin_url('plugin-install.php?tab=commercial'));    
            }else{
                $url = esc_url(admin_url('options-general.php?page=installer'));                    
            }            
        }else{
            $url = esc_url(admin_url('plugin-install.php?tab=commercial'));    
        }
        return $url;
    }
    ...
    public function refresh_repositories_data(){
        ...
        foreach($this->repositories as $id => $data){
            ...
            if(is_wp_error($response)){
                $error = sprintf(__("Can't connect to %s", 'installer') . "\n", $data['products']);
                if(WP_DEBUG){
    ...
    public function plugins_renew_warning($plugin_file, $plugin_data, $status){
        ...
        <tr class="plugin-update-tr"><td colspan="<?php echo esc_attr($wp_list_table->get_column_count()); ?>" class="plugin-update colspanchange">
            <div class="update-message">
            <?php 
                echo esc_html($this->_plugins_renew_warnings[$plugin_file]). ' ';
                printf(esc_html__('%sRenew here%s.', 'installer'), 
                    '<a href="' . esc_url($this->menu_url()) . '">', '</a>');
            ?>
            </div>
        </tr>
    ...
    public function download_plugin_ajax_handler(){
        ...
        if(isset($_POST['data'])){
            ...
            $data    = json_decode( $base_d( $_POST['data'] ), true );
            ...
            foreach($repository['data']['packages'] as $package_id => $package){
                ...
                foreach($package['products'] as $product_id => $product){
                    ...
                    if($fprice){
                        ...
                        $cpndata = $base_en(json_encode(array('theme_author' => $match['author_name'], 'theme_name' => $match['theme_name'], 'vlc' => $match['vlc'])));
                ...
                if($plugin_repository) {
                    ...
                    if (empty($site_key) || empty($subscription_data)) {
                        ...
                        if(defined('DOING_AJAX')){ //WP 4.2
                            ...
                            echo '<a href="' . esc_url(admin_url('plugins.php')) . '">' . __('Return to the plugins page', 'installer') . '</a>';
                            echo '</div>';
