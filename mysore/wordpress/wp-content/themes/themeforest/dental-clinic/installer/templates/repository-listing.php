...
<h3 id="repository-<?php echo esc_attr($repository_id) ?>"><?php echo esc_html($repository['data']['name']) ?></h3>
<?php 
...
<table class="widefat otgs_wp_installer_table" id="installer_repo_<?php echo esc_attr($repository_id) ?>">
    ...
    <tr>
        ...
        <td class="otgsi_register_product_wrap" align="center" valign="top">            
            ...
            <div style="text-align: right;">
                <span><?php esc_html_e('Already bought?', 'installer'); ?>&nbsp;</span>
                ...
                <input type="hidden" name="repository_id" value="<?php echo esc_attr($repository_id) ?>">
                <?php esc_html_e('2. Enter your site key', 'installer'); ?>
                <input type="text" size="10" name="site_key_<?php echo esc_attr($repository_id) ?>" placeholder="<?php echo esc_attr('site key') ?>" />
            ...
            <?php if($this->repository_has_expired_subscription($repository_id)): $expired = true; ?>
                <div><p class="installer-warn-box"><?php esc_html_e('Subscription is expired. You need to either purchase a new subscription or upgrade if available.', 'installer') ?></p></div>
            <?php else: ?>
                <?php $this->show_subscription_renew_warning($repository_id, $subscription_type); ?>
            <?php endif; ?>
            
            <div class="alignright">
                <a class="remove_site_key_js button-secondary" href="#" data-repository=<?php echo esc_attr($repository_id) ?> data-confirmation="<?php esc_attr_e('Are you sure you want to unregister?', 'installer') ?>" data-nonce="<?php echo wp_create_nonce('remove_site_key_' . $repository_id) ?>"><?php printf(__("Unregister %s from this site", 'installer'), $generic_product_name) ?></a>&nbsp;            
                <a class="update_site_key_js button-secondary" href="#" data-repository=<?php echo esc_attr($repository_id) ?> data-nonce="<?php echo wp_create_nonce('update_site_key_' . $repository_id) ?>"><?php esc_html_e('Check for updates', 'installer') ?></a>
            </div>
    ...
    <tr id="repository-<?php echo esc_attr($repository_id) ?>_<?php echo esc_attr($package['id']) ?>">
        <td><img width="140" height="140" src="<?php echo esc_url($package['image_url']) ?>" /></td>
        <td>
            <p><strong><?php echo esc_html($package['name']) ?></strong></p>
            ...
            <?php if($package['products']): ?>
                ...
                <ul class="installer-products-list" style="display:inline">
                    <li>
                        <a class="button-secondary" href="<?php echo esc_url($product['url']) ?>"><?php echo esc_html($product['label']) ?></a>
                    </li>
                </ul>
                <?php endforeach; ?>
            <?php endif; ?>
            ...
            <?php if(!empty($package['sub-packages'])): ?>
                ...
                <h5><a class="installer_expand_button" href="#" title="<?php esc_attr_e('Click to see individual components options.', 'installer') ?>"><?php esc_html_e('Individual components', 'installer') ?></a></h5>
                <?php endif; ?>
                ...
                <?php foreach($subpackages as $package): ?>
                    <tr id="repository-<?php echo esc_attr($repository_id) ?>_<?php echo esc_attr($package['id']) ?>">
                        <td><img width="70" height="70" src="<?php echo esc_url($package['image_url']) ?>" /></td>
                        <td>
                            <p><strong><?php echo esc_html($package['name']) ?></strong></p>
                            <p><?php echo esc_html($package['description']) ?></p>
...
<p><i><?php printf(__('This page lets you install plugins and update existing plugins. To remove any of these plugins, go to the %splugins%s page and if you have the permission to remove plugins you should be able to do this.', 'installer'), '<a href="' . esc_url(admin_url('plugins.php')) . '">' , '</a>'); ?></i></p>
