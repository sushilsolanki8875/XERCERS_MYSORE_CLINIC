		...
		obj.methods = { 
			...
			generateField : function (key, field, val, shcd) { 
				...
				case 'rgba':
					...
					fieldContent += '<input type="text" id="' + key + '_' + obj.methods.uniqID + '" name="' + key + '_' + obj.methods.uniqID + '" value="' + new_color + '" class="cmsms_color_field" data-default-color="' + ((field.def !== '') ? def_val[0] : '') + '" data-alpha="true" data-reset-alpha="true" />';
		...
		privateMethods = { 
			...
			attachGeneratedEvents : function () { 
				...
				if ($.inArray('color', obj.methods.eventsArray) !== -1) {
					// Color Picker
					if (cmsms_lightbox.palettes !== '') {
						$('#cmsmsBox_' + obj.methods.uniqID + ' .cmsms_color_field').wpColorPicker( { 
							palettes : 	cmsms_lightbox.palettes.split(',') 
						} );
					} else {
						$('#cmsmsBox_' + obj.methods.uniqID + ' .cmsms_color_field').wpColorPicker();
					}
