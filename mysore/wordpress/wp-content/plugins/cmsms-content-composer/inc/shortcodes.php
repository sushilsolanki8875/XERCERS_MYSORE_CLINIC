...
function cmsms_clients($atts, $content = null) {
	...
	if ($layout == 'slider') {
		...
		(($animation != '' && $animation_delay != '') ? ' data-delay="' . $animation_delay . '"' : '') . 
		'>' . "\n" . 
			'<div id="cmsms_clients_' . $unique_id . '" class="cmsms_clients_slider cmsms_owl_slider owl-carousel' . 
			(($classes != '') ? ' ' . $classes : '') . 
			'"' . 
			'>' . "\n" . 
			$client_out . 
		...
		if ($layout == 'hover') {
			...
			'>' . "\n" . 
				'<script type="text/javascript">' . 
					...
					'} );' . 
				'</script>' . "\n" . 
				'<div id="cmsms_slider_' . $unique_id . '" class="content_slider cmsms_owl_slider owl-carousel' . 
				(($classes != '') ? ' ' . $classes : '') . 
				'">' . "\n";
			...
			if ($gallery_arr != '') {
				$out .= '<div class="cmsms_wrap_more_items">' . 
					'<div class="cmsms_more_gallery_items cmsms_wrap_items_loader">' . 
						'<a href="javascript:void(0);" class="cmsms_button cmsms_gallery_items_loader cmsms_items_loader">' . 
	...
	if ($quote_mode == 'slider') {
		...
		'>' . "\n" . 
			'<script type="text/javascript">' . 
				...
				'} );' . 
			'</script>' . "\n" . 
			'<div id="cmsms_quotes_slider_' . $unique_id . '" class="cmsms_quotes cmsms_quotes_slider cmsms_owl_slider owl-carousel' . 
			(($classes != '') ? ' ' . $classes : '') . 
			'">' . "\n" . 
				$quote_out . 
			'</div>' . "\n" . 
...
function cmsms_twitter($atts, $content = null) { 
	...
	if ($user != '') {
		$out .= '<div class="cmsms_twitter_wrap">' . 
			...
			'<div id="cmsms_twitter_' . $unique_id . '" class="owl-carousel cmsms_owl_slider cmsms_twitter' . 
			(($classes != '') ? ' ' . $classes : '') . 
			'"' . 
			(($animation != '') ? ' data-animation="' . $animation . '"' : '') . 
			(($animation != '' && $animation_delay != '') ? ' data-delay="' . $animation_delay . '"' : '') . 
...
";
		...
		if ($filter !== '') {
			$out .= "<div class=\"cmsms_post_filter_wrap cmsms_items_filter_wrap\">
				<div class=\"cmsms_post_filter cmsms_items_filter\">
					<span class=\"cmsms_post_filter_loader cmsms_items_filter_loader\"></span>
					<div class=\"cmsms_post_filter_block cmsms_items_filter_block\">
						<a class=\"cmsms_post_filter_but cmsms_items_filter_but cmsms_theme_icon_resp_nav button\">
							<span>" . $filter_text . "</span>
						</a>
						<ul class=\"cmsms_post_filter_list cmsms_items_filter_list\">
							<li class=\"current\">
	...
	if ($pagination !== 'disabled') {
		$out .= '<div class="cmsms_wrap_more_posts cmsms_wrap_more_items">';
			...
			if ($pagination == 'pagination' && $query->max_num_pages > 1) {
				...
				$out .= "<div class=\"cmsms_wrap_post_loader cmsms_wrap_items_loader\">
...
";
	...
	if ($filter != '' || $sorting != '') {
		$out .= "<div class=\"cmsms_project_filter_wrap cmsms_items_filter_wrap\">
			<div class=\"cmsms_project_filter cmsms_items_filter\">
				<span class=\"cmsms_project_filter_loader cmsms_items_filter_loader\"></span>";
				
				if ($sorting != '') {
					$out .= "<div class=\"cmsms_project_sort_block cmsms_items_sort_block\">
						<a href=\"#\" name=\"project_name\" title=\"" . $sorting_name_text . "\" class=\"button cmsms_project_sort_but cmsms_items_sort_but cmsms_theme_icon_slide_bottom" . 
						(($orderby == 'name') ? " current" . 
						(($order == 'DESC') ? " reversed" : "") : "") . 
						"\">
							<span>" . $sorting_name_text . "</span>
						</a>
						<a href=\"#\" name=\"project_date\" title=\"" . $sorting_date_text . "\" class=\"button cmsms_project_sort_but cmsms_items_sort_but cmsms_theme_icon_slide_bottom" . 
				...
				if ($filter != '') {
					$out .= "<div class=\"cmsms_project_filter_block cmsms_items_filter_block\">
	...
	if ($pagination !== 'disabled') {
		$out .= '<div class="cmsms_wrap_more_projects cmsms_wrap_more_items">';
		
			if ($pagination == 'pagination' && $query->max_num_pages > 1) {
				$out .= pagination($query->max_num_pages);
			} elseif ($pagination == 'more' && $query->found_posts > $count) {
				$out .= "<div class=\"cmsms_wrap_project_loader cmsms_wrap_items_loader\">
